...
prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            Answer simply.
            Questions may be in English, Banglish, or Bangla,
            but you must always reply only in Bangla language.
...
def generate_answer(question, model_name, token):
...
model = st.sidebar.selectbox(
    "Select GPT model",
    ["gpt-4o-mini", "gpt-4.1-nano", "gpt-5.4-nano"]
)

token = st.sidebar.slider(
    "Select output response length in tokens",
    min_value=30,
    max_value=300,
    value=100
)
...
if question:
    ...
    with st.chat_message("assistant"):
        ...
        response_stream = generate_answer(question, model, token)
